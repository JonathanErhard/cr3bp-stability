...
namespace cr3bp_benchmarks{
    ...
    std::vector<numeric_error> hamiltonian_conservation_benchmark(const std::vector<std::pair<double,state_type>>& trajectory, double mu){
        ...
        Eigen::Matrix<double,6,1> starting_state_eigen = (Eigen::VectorXd(6) << starting_state_arr[0],starting_state_arr[1],starting_state_arr[2],starting_state_arr[3],starting_state_arr[4],starting_state_arr[5]).finished();
        double starting_hamiltonian = CR3BP::calculate_hamiltonian_rotating(starting_state_eigen,mu);
        // std::cout << starting_state_eigen << "\n###########\n";
        // std::cout << CR3BP::calculate_jacobian_rotating(starting_state_eigen,mu) << '\n';
        ...
        for(const auto& [time,state_arr] : trajectory){
            ...
            double abs_error = ham - starting_hamiltonian;
            double rel_error = abs_error/starting_hamiltonian;
