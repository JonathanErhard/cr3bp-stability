...
int main(){
    ...
    state <<    -0.8269,
                ...
                0.55,
                0.0;
    Eigen::Matrix<double,6,1> barycenter = Eigen::Matrix<double,6,1>::Zero();
    ...
    auto ham = calculate_hamiltonian_rotating<double>;
    std::cout << ham(state,mu) << '\n';
    std::cout << ham(barycenter,mu) << '\n';
    std::cout << ham(state,mu) - ham(barycenter,mu) << '\n';
