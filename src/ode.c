...
/*
  ...
  Input:
    ...
    void F ( double T, double Y[], double YP[] ), the user-supplied function
    ...
    double Y[NEQN], the current solution.

    double *T, the current value of the independent
    variable.

    double TOUT, the desired value of T on output.

    double RELERR, ABSERR, the relative and absolute error
    tolerances.  At each step, the code requires
      abs ( local error ) <= abs ( Y ) * RELERR + ABSERR
    for each component of the local error and solution vectors.

    int *IFLAG, indicates the status of integration.
    IFLAG is normally 1 (or -1 in the special case where TOUT is
    not to be exceeded.) 

    Workspace, double YY[NEQN], used to hold old solution data.

    double WT[NEQN], the error weight vector.
    ...
    int *PHASE1, indicates whether the program is in the
    ...
    double *X, a "working copy" of T, the current value
    of the independent variable, which is adjusted as the code attempts
    to take a step.

    double *H, the current stepsize.

    double *HOLD, the last successful stepsize.

    int *START, is TRUE on input for the first step.

    double *TOLD, the previous value of T.

    double *DELSGN, the sign (+1 or -1) of TOUT - T.

    int *NS, the number of steps taken with stepsize H.

    int *NORND, ?

    int *K, the order of the current ODE method.

    int *KOLD, the order of the ODE method on the previous step.

    int *ISNOLD, the previous value of ISN, the sign
    of IFLAG.

  Output:

    double Y[NEQN], the updated solution.

    double *T, the updated value of the independent
    variable.

    int *IFLAG, indicates the status of integration.
    On normal IFLAG is 2.  Other output values are:
  ...
  Local:
    ...
    integer MAXNUM, the maximum number of steps allowed in one
    call to DE.
*/
{
  double absdel;
...
/*
  ...
  Input:
    ...
    double X, the point where the solution has been computed.

    double Y[NEQN], the computed solution at X.

    double XOUT, the point at which the solution is desired.

    int NEQN, the number of equations.

    int KOLD, the order used for the last
    successful step.

    double PHI[NEQN*16], contains information about the
    interpolating polynomial.

    double PSI[12], contains information about the
    interpolating polynomial.

  Output:

    double YOUT[NEQN], the solution at XOUT.

    double YPOUT[NEQN], the derivative of the solution
    at XOUT.
*/
{
  double eta;
...
/*
  ...
  Discussion:
    ...
    * IFLAG, an indicator to initialize the code.  Normal input
    ...
    code on and so must be variables in the calling program.
  ...
  Input:
    ...
    int *IFLAG, indicates the status of integration.
    On IFLAG is normally 1 (or -1 in the special case where TOUT is
    not to be exceeded.)

    double WORK[100+21*NEQN], workspace.

    int IWORK[5], workspace.

  Output:

    double Y[NEQN], the updated solution.

    double *T, the updated value of the independent variable.
...
/*
  ...
  Input:
    ...
    double *X, the value of the independent variable.

    double Y[NEQN], the approximate solution at the
    ...
    double *H, the suggested stepsize.

    double *EPS, the local error tolerance.

    double WT[NEQN], the vector of error weights.

    int *START, is set to TRUE before the first step.
    The program initializes data, and resets START to FALSE.

    double *HOLD, the step size used on the last
    successful step.

    int *K, the appropriate order for the next step.

    int *KOLD, the order used on the last
    successful step.
    ...
    int *NS, the number of steps taken with
    stepsize H.
  ...
  Output:
    ...
    double *X, the updated value of the independent variable.
    ...
    int *CRASH, is set to TRUE if no step can be taken.
